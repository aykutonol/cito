...
public:
    ...
    void linDyn(const mjData* dMain, const eigVd uMain, mjtNum* Fxd, mjtNum* Fud);

private:
    /// This function sets xNew to the integration of data given a control input
    void copyTakeStep(const mjData* dMain, const eigVd u, mjtNum* xNew);
    /// This function calculates central differences by taking full steps
    void hardWorker(const mjData* dMain, const eigVd uMain, mjtNum* deriv);
    /// MuJoCo model
    const mjModel* m;
