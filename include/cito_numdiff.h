...
public:
    ...
    void linDyn(const mjData* dMain, const eigDbl uMain, mjtNum* Fxd, mjtNum* Fud);

private:
    /// This function sets xNew to the integration of data given a control input
    void copyTakeStep(const mjData* dMain, const eigDbl u, mjtNum* xNew);
    /// This function calculates central differences by taking full steps
    void hardWorker(const mjData* dMain, const eigDbl uMain, mjtNum* deriv);
    /// MuJoCo model
    const mjModel* m;
    /// Perturbation for central differences
    double eps = 1e-6;
    /// Variables for differentiation
    eigMjc xNewTemp, xNewP, xNewN;
    eigDbl uTemp;
    /// Objects
    CitoParams  cp;
    CitoControl cc;
